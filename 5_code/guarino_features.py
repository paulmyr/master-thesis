...
def print_features(features: GuarinoFeatures, name: str = "Features"):
    """Print feature values in a readable format."""
    print(f"\n{name}:")
    print(f"  Spike count:     {float(features.n_spikes):.2f}")
    print(f"  t_first_spike:   {float(features.t_first_spike):.2f} ms")
    print(f"  t_second_spike:  {float(features.t_second_spike):.2f} ms")
    print(f"  t_third_spike:   {float(features.t_third_spike):.2f} ms")
    print(f"  t_last_spike:    {float(features.t_last_spike):.2f} ms")
    print(f"  inv_first_ISI:   {float(features.inv_first_isi):.4f} 1/ms")
    print(f"  inv_last_ISI:    {float(features.inv_last_isi):.4f} 1/ms")
    print(f"  firing_freq:     {float(features.firing_frequency):.2f} Hz")
    print(f"  v_stim_end:      {float(features.v_stim_end):.2f} mV")
    print(
        f"  has_spikes:      [{float(features.has_first_spike):.2f}, "
        f"{float(features.has_second_spike):.2f}, "
        f"{float(features.has_third_spike):.2f}]"
    )
