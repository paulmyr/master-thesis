"""Tests for loss.guarino — hard detection and clip_trainable_params."""

import jax.numpy as jnp
import numpy as np
import pytest

from loss.guarino import (GuarinoFeatures, clip_trainable_params,
                          detect_spikes_hard, extract_experimental_features)
...
class TestClipTrainableParams:
    def test_within_bounds_unchanged(self):
        params = [{"AdEx_g_L": jnp.array([10.0])}, {"AdEx_a": jnp.array([2.0])}]
        bounds = {"g_L": (1.0, 50.0), "a": (0.0, 10.0)}
        clipped = clip_trainable_params(params, bounds)
        assert float(clipped[0]["AdEx_g_L"].item()) == pytest.approx(10.0)
        assert float(clipped[1]["AdEx_a"].item()) == pytest.approx(2.0)

    def test_out_of_bounds_clipped(self):
        params = [{"AdEx_g_L": jnp.array([999.0])}]
        bounds = {"g_L": (1.0, 50.0)}
        clipped = clip_trainable_params(params, bounds)
        assert float(clipped[0]["AdEx_g_L"].item()) == pytest.approx(50.0)

    def test_prefix_stripping(self):
        """The function should strip AdEx_ to look up bounds."""
        params = [{"AdEx_E_L": jnp.array([-200.0])}]
        bounds = {"E_L": (-90.0, -50.0)}
        clipped = clip_trainable_params(params, bounds)
        assert float(clipped[0]["AdEx_E_L"].item()) == pytest.approx(-90.0)
