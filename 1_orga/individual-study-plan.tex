...
\begin{itemize}
    \item Implement AdEx model using a bio-physical neuron simulator that allows automatic differentiation (Prio: high, done).
    \item Extend AdEx implementation to allow for surrogate gradients (Prio: high, done).
    \item Design differentiable loss function that allows successful gradient based optimization (Prio: high, done).
    \item Implement testing framework that allows optimization on artificial experimental trace (Prio: high, todo).
    \item Implement testing framework that allows optimization on single experimental trace (Prio: high, done).
    \item Implement testing framework that allows optimization on multiple experimental traces (Prio: medium, todo).
    \item Compare gradient-based strategy against feature based state-of-the-art genetic / non-differentiable alternatives (Prio: high --- main thesis goal, todo).
    \item Measure computational efficiency of optimization method (Prio: low, todo)
\end{itemize}
...
\begin{itemize}
  \item Feature-based loss adapted from Guarino et al. [9], capturing spike timing, interspike intervals, firing rate, and subthreshold behavior (some version of this used by Deistler)
  \item Test (soft) dynamic time warping as loss function alternative (used by Deistler for fine-tuning).
  \item Van Rossum distance, interesting because of it's differentiable nature
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Comparison with existing methods:} Only selected baseline methods (Nelder-mead, grid search) will be compared. 
      Comprehensive benchmarking against all published optimization approaches is not feasible.
      BluePyOpt for GA might be interesting if there is time.
