...
We investigate two?\merror{how many exactly?\\add softdtw?\\add mse?} different loss functions (Van Rossum and Feature-Based),
analyse (hyper-) parameter sensitivity and evaluate the quality of generated fits using the coincidence factor $\Gamma$ \cite{Jolivet2008} as primary metric.
Additionally, we implement and discuss different training stabilisation techniques.
...
The hyperparameter $\beta$ controls the sharpness: higher values produce steeper gradients at the threshold,
while lower values spread the gradient over wider voltage ranges.

\subsection{Implementation Details}\label{sec:method:surrogate-gradients:implementation}
...
In the differentiable variants, we use the continuous formulation described in Section~\ref{sec:method:surrogate-gradients:implementation} for both $V$ and $w$:
\begin{align}
...
More on trainability of the parameters in section ~\ref{} \itodo{Add result section}
...
Why does this work?
As stated in \ref{sec:adex_impl}, there are two different options on how voltage equations are solved in \texttt{Jaxley}: \texttt{update\_states} and \texttt{compute\_current}.
...
This behaviour needs to be treated when thinking about loss functions.
...
\begin{itemize}
  ...
  \item \textbf{Spike timing:} Maximum absolute difference in spike times
    $$\max_i |t_i^{\text{Jaxley}} - t_i^{\text{Brian2}}| < 1\text{ms.}$$
\end{itemize}
...
We evaluated gradient-based parameter fitting for the AdEx model using \texttt{Jaxley} across four loss functions, multiple surrogate gradients and several training stabilization techniques.
We find that --- while being able to match spike counts and somewhat imitate spiking patterns, it consistently fails to match high precision spike timing across all tested configurations.
Through loss function analysis and gradient diagnostics, we can identify six distinct failure modes that explain this behaviour.
All gradient approaches were consistently outperformed by a Nelder-Mead baseline approach, which was able to capture spike timing precisions reliably.
