...
To bridge this mismatch, we assign a fixed fictitious cylindrical geometry with surface area $A = 100\;\mu\text{m}^2$ (radius $= 1\;\mu$m, length $= 50/\pi\;\mu$m) and store the absolute capacitance $C_m$ (in pF) directly in Jaxley's \texttt{capacitance} parameter.
This works because:

\begin{itemize}
  \item \textbf{Internal dynamics} are handled entirely in \texttt{update\_states}, which computes
    $\mathrm{d}V/\mathrm{d}t = (-I_\text{leak} + I_\text{exp} - w) / C_m$
    in point-neuron units (pA\,/\,pF\,$=$\,mV/ms). Jaxley's voltage solver sees zero channel current (\texttt{compute\_current} returns 0), so there is no double integration.

  \item \textbf{External current} passes through Jaxley's \texttt{convert\_point\_process\_to\_distributed}: $I_\text{nA} / A \times 10^5\;[\mu\text{A/cm}^2]$.
    With $A = 100\;\mu\text{m}^2$ this simplifies to $I_\text{nA} \times 1000 = I_\text{pA}$ (numerically).
    The voltage solver then divides by \texttt{capacitance}: $I_\text{pA}\,/\,C_m\;[\text{mV/ms}]$, which is the correct point-neuron result.

  \item \textbf{$C_m$ is trainable.} Because the geometry is fixed, changing $C_m$ during optimisation only affects the two divisions described above --- exactly the physical role of membrane capacitance.
    No geometry recalculation is needed.

  \item \textbf{Current conversion} from the user side reduces to a simple unit change: $I_\text{nA} = I_\text{pA}\,/\,1000$.
    No parameter-dependent scaling is required.
\end{itemize}
