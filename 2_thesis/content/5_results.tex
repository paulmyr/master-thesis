...
\Cref{sec:results:training} reports what happens when an optimiser is run on those surfaces: two worked examples (one synthetic, one on a real recording), a recovery-radius benchmark over $2{,}700$ paired runs that compares three differentiable losses against two Nelder--Mead baselines on identical perturbed initialisations, and a companion subthreshold benchmark that isolates gradient descent from the spike mechanism entirely.
...
The spike counts logged alongside each timing (last column of \cref{tab:walltime}) confirm that the two simulators integrate the same model throughout: the largest disagreement anywhere in the sweep is $2$ spikes out of $507$ ($\leq 1.1\,\%$), and it grows with trace length, consistent with slow phase drift between two independent but individually converged integrators rather than a model discrepancy.
...
\begin{figure}[t]
  ...
  \emph{C++ loop} is the standalone binary's self-reported integration time (the fair compute-vs-compute analog of Jaxley's warm time); \emph{run() wall} additionally pays per-call process spawn and result I/O.
  ...
  \textbf{(b)} The corresponding Jaxley speedup $t_\mathrm{Brian2}/t_\mathrm{Jaxley}$ (log $y$) against each configuration.
  The measured advantage spans two orders of magnitude --- from essentially parity with the compiled C++ loop to $\approx\!300\times$ against default runtime --- purely by how Brian2 is configured and timed.}\label{fig:walltime-scaling}
\end{figure}
...
Its \texttt{cpp\_standalone} device compiles the whole simulation to a standalone C++ program with no Python in the loop, using Brian2's own default optimised build flags.
Against that best case the fair compute-vs-compute comparison --- Jaxley's warm in-process time against the compiled binary's self-reported integration loop --- is essentially a tie: Jaxley ranges from $1.0\times$ at short durations to $0.9\times$ (i.e.\ marginally slower than the C++ loop) at $5000$\,ms.
The end-to-end \texttt{run()} wall of the standalone binary, which re-executes the compiled program and pays OS process spawn plus result-file I/O on every call, sits between these extremes at $1.9$--$20.5\times$.
The defensible statement is therefore that the Jaxley \gls{AdEx} implementation is between roughly on par with (compiled C++) and two orders of magnitude faster than (default runtime) Brian2, depending entirely on Brian2's configuration.
The point of this benchmark is not that Jaxley out-computes an optimised C compiler on a scalar loop --- it does not --- but that it reaches comparable raw throughput while remaining, unlike Brian2, end-to-end differentiable, batchable via \texttt{vmap}, and GPU-portable.
It is precisely those properties, not a raw-speed advantage, that the gradient-based optimisation in the remainder of this chapter depends on.
...
The recovery-radius benchmark leaves an ambiguity: when a gradient method loses to Nelder--Mead, is that because gradient descent is the weaker optimiser, or because the surrogate gradient through the spike mechanism is a poor search direction?
...
The verdict reverses that of the spiking benchmark: with no spikes in the way, gradient descent is the stronger optimiser, not the weaker one.
...
The efficiency comparison is the central number, and the axis on which it must be read is the number of loss evaluations --- each of which is one forward \gls{AdEx} simulation, the dominant cost of any fit --- rather than raw wall-clock time.
Both optimisers are given the same budget of $25$ evaluations, one Adam step counting as one forward-and-backward pass against one Nelder--Mead function evaluation.
On this axis gradient descent is decisively more sample-efficient: it reaches the convergence tolerance in a median of $9$ fewer evaluations than Nelder--Mead (Wilcoxon $p < 10^{-4}$, lower in $80\,\%$ of pairs), while also reaching a lower final loss (median final \gls{MSE} $0.027$\,mV$^2$ against $0.091$\,mV$^2$; paired difference $-0.054$\,mV$^2$, better in $77\,\%$ of pairs, $p < 10^{-4}$).
...
Wall-clock time runs the other way on this three-parameter toy problem --- the backward pass makes a gradient evaluation more costly than a bare loss evaluation, so at a matched evaluation budget gradient descent takes longer in absolute seconds --- but that gap is an artefact of the trivial problem size, where autodiff overhead has nothing to amortise against, and it inverts as soon as the per-simulation cost dominates the per-step bookkeeping.
...
Measured as bound-normalised parameter error, gradient descent is if anything slightly worse than Nelder--Mead (median $0.081$ vs $0.052$; better in only $24\,\%$ of pairs, and the paired test does not favour it), even though it wins decisively on the loss it actually minimises (\cref{fig:subthreshold-fit-quality}).
...
A better optimiser finds a lower point in that valley without necessarily landing closer to the true parameters.
The subthreshold problem is therefore well-behaved as an \emph{optimisation} problem and simultaneously ill-posed as an \emph{identification} problem --- a distinction we return to in \cref{ch:discussion}.
...
Right: mean bound-normalised parameter-recovery error --- the two methods are indistinguishable, with gradient descent if anything slightly worse.
