...
with non-linear ion channel dynamics (\gls{HH}, stoch. \mtodo{This was an annotation by Alex, what does stoch. mean? Stochastic models?})
...
\mtodo{Loose a word abour point neuron-models (see Alex annotation).}
...
The remainder of this section describes the mechanism; how it transfers to biophysical parameter fitting is taken up in \cref{sec:rw:sg-snn}.
...
as $\gamma \to 0$, Soft-DTW recovers the original DTW distance, while larger $\gamma$ produces smoother gradients at the cost of a looser alignment.\footnote{This is the equivalent to the inverse temperature $\beta$ from the softmax function (\ref{eq:softmax})}
