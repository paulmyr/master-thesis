...
This chapter reports four classes of empirical results, bearing on the thesis's central question.
...
gradient recovery clears the no-optimization floor only at the smallest perturbations, and never matches Nelder--Mead on the same objective.
...
First, every gradient loss but \gls{MSE} clears the floor at small $\delta$ (median $\Gamma_{0.05}=1.00$ for \texttt{grad\_vanrossum}), then collapses by $\delta=0.1$.
...
Fourth, the two Nelder--Mead variants are indistinguishable: \texttt{nm\_guarino} (soft features) and \texttt{nm\_guarino\_hard} (exact features) report identical $\delta_{50}=0.8$ and identical median $\Gamma_{0.05}=+1.00$ on every scenario, with per-direction outcomes differing by at most one direction out of 30.
...
\texttt{grad\_guarino}     & $0.05$ & $1.00$ & $0.05$ & $0.54$ & $0.05$ & $0.90$ \\
\texttt{grad\_vanrossum}   & $0.05$ & $1.00$ & $0.1$  & $1.00$ & $0.05$ & $1.00$ \\
\texttt{nm\_guarino}       & $0.8$  & $1.00$ & $0.8$  & $1.00$ & $0.8$  & $1.00$ \\
\texttt{nm\_guarino\_hard} & $0.8$  & $1.00$ & $0.8$  & $1.00$ & $0.8$  & $1.00$ \\
\midrule
