...
Guarino usually improved on the floor, but at its worst only tied it.
Van Rossum, the best of the three, enlarged the basin only on \texttt{adaptation}.
...
If bias simply grew with the number of softening layers, Van Rossum would beat Guarino everywhere.
It does not.
Van Rossum gives the single best gradient result in the benchmark, $\Gamma_{0.05}=+0.62$ on \texttt{adaptation}, the only gradient row whose recovery radius clears the initialization floor.
But Guarino edges it on \texttt{tonic} ($+0.13$ vs $+0.07$) and \texttt{initial\_bursting} ($+0.19$ vs $+0.12$), where neither loss beats the floor.
So the ordering is not clean.
Van Rossum is the least biased of the three only where the gradient signal is strong enough to separate the losses at all.
...
\subsection{Synthesis and forward directions}\label{sec:discussion:forward}
