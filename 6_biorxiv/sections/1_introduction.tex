...
\Cref{ch:methods} describes how we optimize and evaluate the \gls{AdEx} model: the loss functions we compare, the training setup, and the recovery-based benchmarks.
It closes with our first contribution (\cref{ch:implementation}) --- a differentiable \gls{AdEx} model in \texttt{Jaxley}, together with the surrogate-gradient variant that makes gradient-based fitting possible.
